...
Object.keys(alias.alias).forEach(item => {
  fs.readFile('./tsconfig.json', 'utf8', (err, data) => {
    ...
    if (match?.[1]) {
      ...
      let absPath = path.resolve(__dirname, `.${typePath}`, `.${item.substring(1)}`);
      // 将 自动生成的\\ 转换成能识别的 /
...
class ConvertAliasPlugin {
  ...
  apply(compiler) {
    ...
    compiler.hooks.emit.tapAsync("ConvertAliasPlugin", (compilation, callback) => {
      // 通过过滤只保留需要处理的文件
